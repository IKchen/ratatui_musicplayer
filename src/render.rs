...
impl Render {
    ...
    pub async fn run(& mut self, app: Arc<Mutex<App>>, action_sender:UnboundedSender<Action>, fft_result: Arc<Mutex<Vec<(String, u64)>>>,total_duration:u64,initaled_lyric:Vec<Lyric>) ->JoinHandle<Result<(), MyError>>{
        ...
        let mut soundlist_clone=Arc::clone(&app).lock().await.sounds_list.clone();

        tokio::spawn(async move {


            let mut filelist=crate::components::filelist::FileListComponent::new(soundlist_clone);
            ...
            while !cancelation_token.is_cancelled() {
                if let act_recv = action_receiver.lock().await.recv().await {
                 //   println!("receive action is {act_recv:?}");
                    let shared_data_clone = Arc::clone(&app).lock().await.log.clone();
                    tracinglog.set_log(shared_data_clone.join("\n"));//读取日志
                    tracinglog.update(Some(Action::Down))?;//来一条消息，翻一条数据
